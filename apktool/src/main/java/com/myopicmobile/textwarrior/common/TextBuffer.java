...
public class TextBuffer implements java.lang.CharSequence
{
	...
	protected List<Pair<ColorScheme.Colorable>> _spans;
	...
	synchronized public int getLineOffset(int lineNumber) {
		...
		Pair<Integer> cachedEntry = _cache.getNearestLine(lineNumber);
	...
	synchronized public int findLineNumber(int charOffset) {
		...
		Pair<Integer> cachedEntry = _cache.getNearestCharOffset(charOffset);
	...
	public void clearSpans() {
		_spans = new Vector<>();
	    _spans.add(new Pair<ColorScheme.Colorable>(0, ColorScheme.Colorable.NAME));
	}

	public List<Pair<ColorScheme.Colorable>> getSpans() {
		return _spans;
	}
	 ...
	 */
	public void setSpans(List<Pair<ColorScheme.Colorable>> spans) {
		_spans = spans;
	}
