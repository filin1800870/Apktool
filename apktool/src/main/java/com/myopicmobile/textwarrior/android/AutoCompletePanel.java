...
public class AutoCompletePanel {
	...
	private Language _globalLanguage = LanguageNonProg.getInstance();
	...
	class MyAdapter extends ArrayAdapter<String> implements Filterable {
		 ...
		 * Implement ingress filtering algorithms that are done automatically
		 */
		@Override
		public Filter getFilter() {
			Filter filter = new Filter() {
				...
				/**
				 * This method is executed in the background thread, defining the filtering algorithm
				 */
				@Override
				protected FilterResults performFiltering(CharSequence constraint) {

					// Filtering here Filter results are returned with FilterResults after filtration
					ArrayList <String>buf = new ArrayList<String>();
					_constraint = _globalLanguage.complete(buf, constraint);
					FilterResults filterResults = new FilterResults();
					filterResults.values = buf;   // Results is the filter above.
					filterResults.count = buf.size();  // Number of results
					return filterResults;
				}
				/**
				 * This method is executed in the UI thread to update the autocomplete list
				 */
				@Override
				protected void publishResults(CharSequence constraint, FilterResults results) {
					if (results != null && results.count > 0 && !_abort.isSet()) {
						// There are filtered results that show a list of auto-completions
						...
						setHeight((int)(Math.min(_textField.getContentHeight() * 0.9f, getItemHeight() * results.count)));
						setWidth((int)(_textField.getContentWidth() * 0.9f));
						setHorizontalOffset((int)(_textField.getContentWidth() * 0.05f) + _textField.getScrollX());
						setVerticalOffset(y - _textField.getHeight());//_textField.getCaretY()-_textField.getScrollY()-_textField.getHeight());
						notifyDataSetChanged();
						show();
					} else {
						// No filtering results, close list
